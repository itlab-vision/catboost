...
float L2SqrDistance_rvv_##MType(const float* a, const float* b, int length) {  \
  ...
  vfloat32##MType##_t v_a, v_b, v_diff, v_mul, v_sum;                  \
                                                                       \
  bs2 = __riscv_vsetvl_e32m1(length);                                  \
  bs = __riscv_vsetvl_e32##MType(length);                              \
  v_sum = __riscv_vfmv_v_f_f32##MType(0, bs);                          \
  vfloat32m1_t v_res = __riscv_vfmv_v_f_f32m1(0, bs2);                 \
  float res;                                                           \
                                                                       \
  bs = __riscv_vsetvl_e32##MType(length);                              \
  for (; length > bs; length -= bs) {                                  \
    v_a = __riscv_vle32_v_f32##MType(a, bs);                           \
    v_b = __riscv_vle32_v_f32##MType(b, bs);                           \
    v_diff = __riscv_vfsub_vv_f32##MType (v_a, v_b, bs);               \
    v_sum = __riscv_vfmacc_vv_f32##MType (v_sum, v_diff, v_diff, bs);  \
    a += bs;                                                           \
    b += bs;                                                           \
                                                                       \
  }                                                                    \
  v_res = __riscv_vfredosum_vs_f32##MType##_f32m1(v_sum, v_res, bs);     \
  if(length > 0)                                                       \
  {                                                                    \
    bs = __riscv_vsetvl_e32##MType(length);                            \
    v_a = __riscv_vle32_v_f32##MType(a, bs);                           \
    v_b = __riscv_vle32_v_f32##MType(b, bs);                           \
    v_diff = __riscv_vfsub_vv_f32##MType(v_a, v_b, bs);                \
    v_mul  = __riscv_vfmul_vv_f32##MType(v_diff, v_diff, bs);          \
    v_res = __riscv_vfredosum_vs_f32##MType##_f32m1(v_mul, v_res, bs);   \
  }                                                                    \
