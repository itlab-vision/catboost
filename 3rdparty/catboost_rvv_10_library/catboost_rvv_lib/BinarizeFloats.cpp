...
{
  size_t bs, m_bs, c_bs;
  bs =  __riscv_vsetvl_e32m4(val_cnt);
  c_bs = val_cnt / bs;
  m_bs = val_cnt % bs;

  vuint8m1_t one_v = __riscv_vmv_v_x_u8m1(1, bs);
  
  vfloat32m4_t val_v;
  vuint8m1_t writePtr_v;
  
  for (size_t docId = 0; docId < val_cnt; docId += bs) {
    val_v = __riscv_vle32_v_f32m4(val_arr + docId, bs);
    
    uint8_t* writePtr = result + docId;
    writePtr_v = __riscv_vle8_v_u8m1(writePtr, bs); 

    for (size_t blockStart = 0; blockStart < cnt_border; blockStart += MAX_VALUES_PER_BIN) {
        const size_t blockEnd = Min(blockStart + MAX_VALUES_PER_BIN, cnt_border);
        for (size_t borderId = blockStart; borderId < blockEnd; ++borderId) {
          vbool8_t val_gt_border_m = __riscv_vmfgt_vf_f32m4_b8 (val_v, borders[borderId], bs);
          writePtr_v = __riscv_vadd_vv_u8m1_mu (val_gt_border_m, writePtr_v, writePtr_v, one_v, bs);
        }
        __riscv_vse8_v_u8m1(writePtr, writePtr_v, bs);
        writePtr += docCount;
    }
  }  
